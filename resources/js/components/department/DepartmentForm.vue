            ...
            <div class="col-md-10">
                <div class="card">
                    <div class="card-header">
                        <div class="d-inline-block"><h1>{{ $t('editDepartment') }}</h1></div>
                    </div>
                    <div class="card-body">
                        <form id="department-form">
                            <div class="form-group">
                                <label class="h6 font-weight-bold">{{ $t('picture') }}:
                                    ...
                                    <image-upload-crop id="logo-image-uploader" field="image"
                                                       ...
                                                       url="/api/admin/upload/images?entity=App\Models\Department"
                            ...
                            <div class="form-group">
                                <label class="h6 font-weight-bold">{{ $t('users') }}:
                                    <multiselect v-model="usersIds" name="users" :options="usersList" label="name" :multiple="true" :preserve-search="true" :placeholder="$t('selectUsers')" :close-on-select="false" track-by="id"></multiselect>
                                </label>
                            </div>
                        </form>
    ...
    methods: {
        ...
        fetchData() {
            axios.get('/api' + this.$route.path).then(response => {
                this.logo  = response.data.department.logo;
                this.name  = response.data.department.name;
                this.name  = response.data.department.name;
                this.description  = response.data.department.description;
                this.usersIds  = response.data.department.users;
                ...
                .catch(error => {
                    ...
                    if(error) {
                        ...
                        if(error.response) {
                            ...
                            this.$root.showErrorPopup(error.response.data.message, errors, 'department-form');
                        }
                    }
                })
        },
        save(e) {
            ...
            let pageUrl;
            if(this.$route.params.id) {
                pageUrl = '/api/admin/departments/' + this.department.id;
            } else {
                pageUrl = '/api/admin/departments';
            }
            axios.post(pageUrl, {
